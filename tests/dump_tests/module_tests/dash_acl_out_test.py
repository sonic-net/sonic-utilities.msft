...
class TestDashAclOutModule:
    def test_working_state(self, match_engine):
        ...
        """
        params = {Dash_Acl_Out.ARG_NAME: "ENI0:1", "namespace": ""}
        m_dash_acl_out = Dash_Acl_Out(match_engine)
        returned = m_dash_acl_out.execute(params)
        expect = create_template_dict(dbs=["APPL_DB"])
        expect["APPL_DB"]["keys"].append("DASH_ACL_OUT_TABLE:ENI0:1")
    ...
    def test_not_working_state(self, match_engine):
        ...
        """
        params = {Dash_Acl_Out.ARG_NAME: "ENI0:2", "namespace": ""}
        m_dash_acl_out = Dash_Acl_Out(match_engine)
        returned = m_dash_acl_out.execute(params)
        expect = create_template_dict(dbs=["APPL_DB"])
        expect["APPL_DB"]["tables_not_found"].append("DASH_ACL_OUT_TABLE")
        ddiff = DeepDiff(returned, expect, ignore_order=True)
        print(returned)
        assert not ddiff, ddiff
