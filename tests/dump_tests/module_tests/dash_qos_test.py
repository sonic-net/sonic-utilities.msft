...
class TestDashQosModule:
    def test_working_state(self, match_engine):
        ...
        """
        params = {Dash_Qos.ARG_NAME: "qos100", "namespace": ""}
        m_dash_qos = Dash_Qos(match_engine)
        returned = m_dash_qos.execute(params)
        expect = create_template_dict(dbs=["APPL_DB"])
        expect["APPL_DB"]["keys"].append("DASH_QOS_TABLE:qos100")
    ...
    def test_not_working_state(self, match_engine):
        ...
        """
        params = {Dash_Qos.ARG_NAME: "qos2", "namespace": ""}
        m_dash_qos = Dash_Qos(match_engine)
        returned = m_dash_qos.execute(params)
        expect = create_template_dict(dbs=["APPL_DB"])
        expect["APPL_DB"]["tables_not_found"].append("DASH_QOS_TABLE")
        ddiff = DeepDiff(returned, expect, ignore_order=True)
        print(returned)
        assert not ddiff, ddiff
