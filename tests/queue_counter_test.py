...
"""
show_queue_counters_port_nonzero = """\
     Port    TxQ    Counter/pkts    Counter/bytes    Drop/pkts    Drop/bytes
---------  -----  --------------  ---------------  -----------  ------------
Ethernet8    UC1              38               17           68            91
Ethernet8    UC2              16               65           79            51
...
show_queue_counters_port_json_nonzero = """\
{
  "Ethernet8": {
    "ALL20": {
      "dropbytes": "N/A",
...
show_queue_counters_json_nonzero = """\
{
  "Ethernet0": {
    "ALL20": {
      "dropbytes": "N/A",
...
class TestQueue(object):
    ...
    def test_queue_counters_nonzero(self):
        runner = CliRunner()
        result = runner.invoke(
            show.cli.commands["queue"].commands["counters"],
            ["--nonzero"]
        ...
        print(result.output)
        assert result.exit_code == 0
        assert result.output == show_queue_counters_nonzero
    ...
    def test_queue_counters_port_nonzero(self):
        runner = CliRunner()
        result = runner.invoke(
            show.cli.commands["queue"].commands["counters"],
            ["Ethernet8", "--nonzero"]
        ...
        print(result.output)
        assert result.exit_code == 0
        assert result.output == show_queue_counters_port_nonzero
    ...
    def test_queue_counters_json_nonzero(self):
        runner = CliRunner()
        result = runner.invoke(
            show.cli.commands["queue"].commands["counters"],
            ["--json", "--nonzero"]
        ...
        for _, v in json_output.items():
            del v["time"]
        assert json_dump(json_output) == show_queue_counters_json_nonzero 
    ...
    def test_queue_counters_port_json_nonzero(self):
        runner = CliRunner()
        result = runner.invoke(
            show.cli.commands["queue"].commands["counters"],
            ["Ethernet8", "--json", "--nonzero"]
        )
        assert result.exit_code == 0
        print(result.output)
        json_output = json.loads(result.output)
        ...
        for _, v in json_output.items():
            del v["time"]
        assert json_dump(json_output) == show_queue_counters_port_json_nonzero
